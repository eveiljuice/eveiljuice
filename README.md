# Timo Elsesser

Product builder · AI enthusiast · [timoelsesser.com](https://timoelsesser-site.pages.dev/)

---

## About

I build AI agents, developer tools, and small internet products — mostly in public, mostly alone. Im founder of **@Elsesser Ind**.

## Why I build

The most interesting software shift of this decade is *agents becoming real collaborators* — not chat boxes, not autocomplete, but processes that actually execute on a user's behalf. I'm trying to build the primitives that shift needs: a browser agents can drive, an arena where they can compete, an activity page for what they do, guardrails so they can be trusted with destructive actions.

I'm not the most classically trained engineer, and I'm fine with that. What I care about is building useful things in software and helping non-technical founders actually ship — like [speaky-chinese-ai](https://github.com/eveiljuice/speaky-chinese-ai), a language-learning product I helped get off the ground without its creator needing to write a line of code.

I ship small, in public, often imperfect. One weekend repo with a real user beats six months of planning.

## Projects

### Agent-native primitives — the Claw ecosystem

**[clawbrowser](https://github.com/eveiljuice/clawbrowser)** — The true browser for your agents. CLI-first, DevTools-powered, stdout-friendly.

**[clawgames](https://github.com/eveiljuice/clawgames)** — Open platform where AI agents compete in turn-based games via REST API.

**[clawpage](https://github.com/eveiljuice/clawpage)** — GitHub-style activity heatmap, but for your AI companions.

**[claw-guardrails](https://github.com/eveiljuice/claw-guardrails)** — OpenClaw plugin that enforces multi-layer permission checks before potentially destructive actions.

**[openclaw-browser](https://github.com/eveiljuice/openclaw-browser)** — Minimal desktop browser with an OpenClaw AI sidebar. Rust + wry + tao.

**[openclaw-doctor](https://github.com/eveiljuice/openclaw-doctor)** — Autonomous DevOps agent that diagnoses and fixes OpenClaw Gateway issues.

### AI platforms & products

**[lord-bots](https://github.com/eveiljuice/lord-bots)** — BaaS platform where an AI agent writes Telegram bots end-to-end. Claude Agent SDK + Bot API 9.6 Managed Bots.

**[gauntlet-ai](https://github.com/eveiljuice/gauntlet-ai)** — A modern AI app builder.

**[trendxl](https://github.com/eveiljuice/trendxl)** — AI-powered trend analysis platform.

**[candidate-search-ai-agent](https://github.com/eveiljuice/candidate-search-ai-agent)** — Autonomous AI agent that finds developers across any platform via natural-language queries.

**[businessclaw-landing](https://github.com/eveiljuice/businessclaw-landing)** — BusinessClaw — an AI assistant for entrepreneurs.

### Developer tooling

**[claude-plugin-design-md](https://github.com/eveiljuice/claude-plugin-design-md)** — Claude Code plugin and `npx` CLI that scaffolds `DESIGN.md` files following Google's spec for AI-readable design systems. Published on [npm](https://www.npmjs.com/package/claude-plugin-design-md).

**[tg-cli](https://github.com/eveiljuice/tg-cli)** — Full-featured Telegram client in the terminal. Python + Telethon + Textual.

**[vscode-toon](https://github.com/eveiljuice/vscode-toon)** — TOON language extension for VSCode.

**[json-to-toon](https://github.com/eveiljuice/json-to-toon)** — Web converter from JSON to TOON. Vite.

**[htmx-go-starter](https://github.com/eveiljuice/htmx-go-starter)** — Starter template for HTMX + Go projects.

### Telegram automation

**[radar](https://github.com/eveiljuice/radar)** — Telegram bot that catches fresh property listings before competitors.

**[GOreposearcher](https://github.com/eveiljuice/GOreposearcher)** — Telegram bot that searches Go repositories on GitHub.

### Products & sites

**[master-corner](https://github.com/eveiljuice/master-corner)** — Handmade-goods marketplace connecting artisans with buyers.

**[elsesserandco-website](https://github.com/eveiljuice/elsesserandco-website)** — Website for a real-estate agency in Yekaterinburg.

**[theopendashboard](https://github.com/eveiljuice/theopendashboard)** — Open dashboard for tracking personal transactions.

**[timoelsesser-site](https://github.com/eveiljuice/timoelsesser-site)** — Personal site and logbook. Astro + Bun.

## Elsewhere

- **web** · [timoelsesser.com](https://timoelsesser.com)
- **x** · [@Timo1153](https://x.com/Timo1153)
- **telegram** · [@promptdie](https://t.me/promptdie)
- **email** · [mannitiger13@gmail.com](mailto:mannitiger13@gmail.com)
